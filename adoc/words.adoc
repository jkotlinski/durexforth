...
((2over)) _( a b c d -- a b c d a b )_ :: Copies cell pair _a_ _b_ to top of stack.
((2swap)) _( a b c d -- c d a b )_ :: Exchanges the top two cell pairs.
((>number)) _( ud addr u -- ud addr2 u2 )_ :: Converts the string in _addr u_ to digits, using `base`, and adds each digit into _ud_ after multiplying it with `base`. _addr2 u2_ contains the part of the string that was not converted.
((>body)) _( xt -- addr )_ :: Returns the data field address that belongs to the execution token. Example use: `' foo >body`
...
((top!)) _( addr -- )_ :: Relocates the dictionary to _addr_. Example:
...
((save-pack)) _( "filename" -- )_ :: Saves a compact version of forth to the given _filename_.
((save-prg)) _( "filename" -- )_ :: Saves a forth program with no dictionary to _filename_.
