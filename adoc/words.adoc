...
((within)) _( n lo hi -- flag )_ :: Return true if lo &le; n < hi.
...
[[pad]] ((pad)) _( -- addr )_ :: _addr_ is the address of the `pad`, a 161-byte memory region that can be used freely by user words.
...
[[find]] ((find)) _( cstr -- cstr 0 | xt -1 | xt 1 )_ :: Find the definition named in the counted string _cstr_.
...
((device)) _( device# -- )_ :: Switch the active device.
