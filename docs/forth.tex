...
\item[no-tce] Do not perform tail call elimination (the practice of substituting jsr/rts with jmp) if the last compiled word is the jsr target.
...
\item[>number ( ud addr u -- ud2 addr2 u2)] Converts the string in \texttt{addr u} to digits, using \texttt{BASE}, and adds each digit into ud after multiplying it with \texttt{BASE}. \texttt{addr2 u2} contains the part of the string that was not converted.
