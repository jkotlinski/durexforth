...
\item[key ( -- n )] Reads a character from input.
...
\item[compile-ram (value)] It is possible to make DurexForth interpret from RAM
by setting this value. E.g. \texttt{c000 compile-ram !} will make DurexForth
interpret from \texttt{c000}. The string to interpret must be terminated with
the bytes \texttt{20 00}.
...
Example: \texttt{loc words >cfa execute} equals \texttt{words}.
\end{description}
