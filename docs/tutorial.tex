...
\begin{enumerate}
\item Issue \index{-{-}-modules-{-}-}\texttt{---modules---} to unload all modules, or \index{-{-}-editor-{-}-}\texttt{---editor---} to unload the editor only.
...
You may launch a word automatically at start-up by setting the variable \index{start}\texttt{start} to the execution token of the word.  Example: \texttt{' megademo start !} To save the new configuration to disk, type e.g. \texttt{save-forth megademo}.
...
\subsection{Turn-key Operation}\label{Turn-key Operation}
