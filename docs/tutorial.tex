...
Now some arithmetics. The default input mode in DurexForth is hexadecimal. As an example, \texttt{1000 a * u.} will calculate $\mathtt{a \times 1000}$ and print the result $\mathtt{a000}$. If you wish, it is possible to switch numerical base using \texttt{decimal} and \texttt{hex}.
...
If you need to flash as fast as possible, it is possible to use the durexForth assembler to generate machine code. \texttt{code} and \texttt{;code} define a code word, just like \texttt{:} and \texttt{;} define Forth words. Within a code word, you can use assembler mnemonics. 
