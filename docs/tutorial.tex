...
Programs can be saved in a compacted state using \texttt{save-prg}. These programs are stored with 32 bytes between \texttt{here} and \texttt{latest}. When they are first loaded, they will restore the header space to its original \texttt{top}.

If you have developed a program that has no further need of the interpreter, you can eliminate the dictionary entirely before calling \texttt{save-forth} or \texttt{save-prg} with \texttt{top to latest}. Now, the header space is 1 byte at \texttt{top}. If you expect that you may overwrite the original \texttt{top} byte, you can reposition it to a constant value within your code space.

\begin{verbatim}
here 0 , top to latest top! save-prg @0:turnkey
\end{verbatim}

This allows your program to use memory down to \texttt{here} plus 32 bytes for safety. Note that the above command cannot be executed from a prompt, as \texttt{top to latest} immediately removes all word headers.
