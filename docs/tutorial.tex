...
\section{Start-Up Control}

At start-up, the file \texttt{modules} will be executed. By default, it loads the following optional modules:

\begin{verbatim}
s" debug" load # debugger
s" asm" load # assembler
s" edit" load # editor
\end{verbatim}

To decrease boot time and RAM usage, you may selectively choose which modules you want to load at start-up. Simply open \texttt{modules} in the editor (using \texttt{s" modules" edit}) and comment out the modules you are not interested in. You may also customize this file to add your own modules to be loaded at start-up.
