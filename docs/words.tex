...
\begin{description}
    \item[open ( filenameptr filenamelength file-number secondary-addr -- )] Open a file.
    \item[chkin ( file-number -- )] Selects a file as the current input device.
    \item[chkout ( file-number -- )] Selects a file as the current output device.
    \item[clrch ( -- )] Resets input and output to the keyboard and screen.
    \item[readst ( -- status )] Returns the status of the last IO operation. For serial devices, \texttt{\$01} = write timeout, \texttt{\$02} = read timeout, \texttt{\$40} = end of file (EOI), \texttt{\$80} = device not present.
...
\texttt{open}, \texttt{chkin} and \texttt{chkout} will close the file and abort with an error message if an error is encountered. If you want to handle errors yourself, the words \texttt{open?}, \texttt{chkin?} and \texttt{chkout?} take the same arguments, but return the file number and a Kernal error code, without any error handling:

\begin{description}
    \item[open? ( filenameptr filenamelength file-number secondary-addr -- file-number result )] Open a file, without error handling.
    \item[chkin? ( file-number -- file-number result )] Selects a file as the current input device, without error handling.
    \item[chkout? ( file-number -- file-number result )] Selects a file as the current output device, without error handling.
\end{description}
