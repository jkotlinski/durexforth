...
\begin{description}
    \item[open ( filenameptr filenamelength secondary-addr file-number -- )] Open a file.
    ...
    \item[readst ( -- status )] Returns the status from the last IO operation.
    \item[chrin ( -- char)] Reads a character from the current input device.
    \item[faccept ( addr u -- u )] Analog of \texttt{accept} that reads from the current input device rather than the keyboard. Receive a string of at most u characters from the current input device, into the buffer that starts at addr.  Returns how many characters were received.
\end{description}

\texttt{open}, \texttt{chkin} and \texttt{chkout} will close the file and abort with the message ``\texttt{io err}'' if an error is encountered. If you want to handle errors yourself, the words \texttt{(open)}, \texttt{(chkin)} and \texttt{(chkout)} take the same arguments, but return 0 on success and nonzero on failure, without any error handling.
