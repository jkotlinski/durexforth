...
[[lsb]] ((lsb)) _( -- addr )_ :: _addr_ points to the top of the LSB parameter stack. Together with the `x` register, it can be used to access stack contents.
[[msb]] ((msb)) _( -- addr )_ :: _addr_ points to the top of the MSB parameter stack. Together with the `x` register, it can be used to access stack contents.
[[w]] ((w)) _( -- addr )_ :: A zero-page cell that code words may use freely as work area.
[[w2]] ((w2)) _( -- addr )_ :: Second zero-page work area cell.
[[w3]] ((w3)) _( -- addr )_ :: Third zero-page work area cell.
