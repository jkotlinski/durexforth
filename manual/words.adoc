...
((throw)) _( n -- )_ :: Throw exception _n_, unless _n_ is 0. If the exception is not caught, print `_n_ err`, empty the data stack and `quit`.
...
((ioabort)) _( ioresult -- )_ Handle error conditions for `open`, `chkin` and `chkout`. On failure, print error message and throw exception -37.
