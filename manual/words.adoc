...
((compare)) _( caddr1 u1 caddr2 u2 -- n )_ :: Compare the bytes of two strings, giving 0 if equal, 1 if the first is larger, -1 if smaller. If they differ in length but are equal up to the shorter, then the longer string is considered larger. This word and the next two are optionally included with `include compare`.
((strcmp)) _( caddr1 u1 caddr2 u2 -- n )_ :: Like `compare`, but gives a difference of bytes or lengths (doesn't convert to 1 or -1).
((memcmp)) _( addr1 addr2 u -- n )_ :: Compare byte arrays of equal length.
