...
((send-cmd)) _( c-addr u -- )_ :: Demonstrates reading the error channel.
((dos)) _( "cmd" -- )_ :: Same as Dos command.
((bsave)) _( start end+1 filenameptr filenamelength -- )_ :: Demonstrates sending a file to disk.
((dir)) :: Same as ``ls``, demonstrates loading a file.
